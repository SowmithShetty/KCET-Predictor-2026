...
# --- CORRECTED: PDF GENERATION FUNCTION (Stripping Emojis to prevent crash) ---
def create_pdf(df_to_print, user_details):
    try:
        ...
        # Clean special characters/emojis from strings
        def clean_str(s):
        ...
        pdf.cell(200, 8, txt=f" Region: {user_details['region']} | Target Branch: {clean_str(user_details['branch'])}", ln=True)
        pdf.ln(5)
        ...
        for _, row in df_to_print.iterrows():
            # Clean college name and status of emojis for PDF compatibility
            c_name = clean_str(row['College'])
            c_status = clean_str(row['Status'])
            
            display_name = (c_name[:60] + '..') if len(c_name) > 60 else c_name
            pdf.cell(110, 10, f" {display_name}", 1)
            pdf.cell(45, 10, f" {row['Safety Range']}", 1)
            pdf.cell(35, 10, f" {c_status}", 1)
            pdf.ln()
            
        pdf.ln(15)
        pdf.set_font("Arial", 'I', 8)
        pdf.multi_cell(0, 5, txt="Disclaimer: These predictions are generated by an AI model based on historical trends (2023-2025). Actual cutoffs for 2026 may vary.", align='C')
...
if st.session_state.top_5_df is not None:
    if isinstance(st.session_state.top_5_df, str):
        ...
        st.success("🎯 Top 5 Best Fits Across All Branches")
...
if col_f1.button("👍 Yes"): st.toast("Thanks for your feedback!")
if col_f2.button("👎 No"): st.toast("We'll work on improving!")
